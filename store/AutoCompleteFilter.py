...
class CustomAutocompleteFilter(admin.SimpleListFilter):
    ...
    def __init__(self, request, params, model, model_admin):
        ...
        widget = AutocompleteSelect(remote_field,
                                    ...
                                    custom_url=self.get_autocomplete_url(
                                        request, model_admin), )
        ...
        if self.is_placeholder_title:
            # Upper case letter P as dirty hack for bypass django2 widget
            # force placeholder value as empty string ("")
    ...
    def get_queryset_for_field(self, model, name):
        ...
        if isinstance(field_desc, ManyToManyDescriptor):
            related_model = field_desc.rel.related_model \
                if field_desc.reverse else field_desc.rel.model
    ...
    def _add_media(self, model_admin, widget):
        ...
        if not hasattr(model_admin, 'Media'):
            raise ImproperlyConfigured(
                'Add empty Media class to %s. '
                'Sorry about this bug.' % model_admin)

        def _get_media(obj):
            return Media(media=getattr(obj, 'Media', None))

        media = _get_media(model_admin) + widget.media + _get_media(
            CustomAutocompleteFilter) + _get_media(self)
